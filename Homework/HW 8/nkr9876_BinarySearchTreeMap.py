class BinarySearchTreeMap:
    ...
    class Node:
        ...
        # These methods take O(h) time to execute, and update the subtree size of all nodes
        # They're only called for actions with complexity >= O(h), though, so they don't impact overall runtime

        def shrinkMe(self):
            self.subtree_size -= 1
            if self.parent is not None:
                self.parent.shrinkMe()
        ...
        def __init__(self, item):
            ...
            # You'd think this should be 1, but because of how growMe and shrinkMe work, 0 is needed for the expected values
            self.subtree_size = 0
    ...
    def insert(self, key, value):
        ...
        # Once everything's connected, call the method
        new_node.growMe()
    ...
    def delete_node(self, node_to_delete):
        ...
        # Before we disconnect the node, let's call the method we made
        node_to_delete.shrinkMe()
    ...
    def get_ith_smallest(self, i):
        ...
        def subtree_get_ith_smallest(node, i):
            if node.left is None:
                left_nodes = 0
            else:
                left_nodes = node.left.subtree_size

            if i <= left_nodes:
                return subtree_get_ith_smallest(node.left, i)
            elif i == left_nodes + 1:
                return node.item.key
            else:
                return subtree_get_ith_smallest(node.right, i - (left_nodes + 1))

        if i > len(self):
            raise IndexError
        else:
            return subtree_get_ith_smallest(self.root, i)
